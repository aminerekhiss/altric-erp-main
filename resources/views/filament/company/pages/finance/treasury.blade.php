<x-filament-panels::page>
    <div class="mb-4 rounded-xl border border-slate-200 bg-white p-4">
        <h2 class="text-base font-semibold text-slate-900">Tresorerie</h2>
        <p class="mt-1 text-sm text-slate-500">Visibilite immediate sur soldes, encaissements et decaissements du mois.</p>
    </div>
    ...
    <div class="grid gap-4 md:grid-cols-3">
        <div class="rounded-xl border border-slate-200 bg-white p-4">
            <p class="text-xs font-semibold uppercase tracking-wide text-slate-500">Comptes bancaires</p>
            <p class="mt-1 text-3xl font-bold text-slate-900">{{ number_format($bankAccountsCount) }}</p>
        </div>
        <div class="rounded-xl border border-emerald-200 bg-emerald-50 p-4">
            <p class="text-xs font-semibold uppercase tracking-wide text-emerald-700">Encaissements (mois)</p>
            <p class="mt-1 text-3xl font-bold text-emerald-900">{{ number_format((float) $depositsMonth, 3, '.', ' ') }} TND</p>
        </div>
        <div class="rounded-xl border border-rose-200 bg-rose-50 p-4">
            <p class="text-xs font-semibold uppercase tracking-wide text-rose-700">Decaissements (mois)</p>
            <p class="mt-1 text-3xl font-bold text-rose-900">{{ number_format((float) $withdrawalsMonth, 3, '.', ' ') }} TND</p>
        </div>
    </div>

    <div class="mt-4 rounded-xl border border-slate-200 bg-white p-4">
        <h3 class="text-base font-semibold text-slate-900">Mouvements de tresorerie recents</h3>
        <div class="mt-3 overflow-auto">
            <table class="min-w-full text-sm">
                ...
                @empty
                    <tr><td colspan="4" class="py-3 text-slate-500">Aucun mouvement.</td></tr>
                @endforelse
                </tbody>
            </table>
        </div>
