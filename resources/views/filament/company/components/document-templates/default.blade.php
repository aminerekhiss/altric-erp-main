<x-company.document-template.container class="default-template-container" :preview="$preview">
            ...
            <div class="space-y-4">
                ...
                <div class="text-sm">
                    ...
                    @if($document->company->taxId)
                        <p>MF: {{ $document->company->taxId }}</p>
                    @endif
                    @if($document->company->vatNumber)
                        <p>TVA: {{ $document->company->vatNumber }}</p>
                    @endif
                    @if($document->company->tradeRegisterNumber)
                        <p>RC: {{ $document->company->tradeRegisterNumber }}</p>
                    @endif
                    @if($document->company->rneNumber)
                        <p>RNE: {{ $document->company->rneNumber }}</p>
                    @endif
                </div>
            </div>
        </div>
    ...
    <x-company.document-template.line-items class="default-template-line-items">
        <table class="w-full text-left table-fixed">
            ...
            @endif
            @if($document->stampDuty)
                <tr>
                    <td class="pl-6 py-2" colspan="2"></td>
                    <td class="text-right py-2">Timbre Fiscal:</td>
                    <td class="text-right pr-6 py-2">{{ $document->stampDuty }}</td>
                </tr>
            @endif
            <tr>
                <td class="pl-6 py-2" colspan="2"></td>
            ...
            @if($document->totalIncludingStamp)
                <tr>
                    <td class="pl-6 py-2" colspan="2"></td>
                    <td class="text-right font-semibold py-2">Total TTC + Timbre:</td>
                    <td class="text-right pr-6 py-2">{{ $document->totalIncludingStamp }}</td>
                </tr>
            @endif
