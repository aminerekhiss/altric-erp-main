        ...
        return Section::make(translate('Legal & Compliance'))
            ->schema([
                ...
                TextInput::make('vat_number')
                    ->label(translate('VAT Number (TVA)'))
                    ->maxLength(50),
                TextInput::make('trade_register_number')
                    ->label(translate('Trade Register Number'))
                    ->maxLength(100),
                TextInput::make('rne_number')
                    ->label(translate('RNE Number'))
                    ->maxLength(100),
                Select::make('tunisian_stamp_duty_enabled')
                    ->label(translate('Apply Tunisian Stamp Duty on Invoices'))
                    ->options([
                        false => translate('No'),
                        true => translate('Yes'),
                    ])
                    ...
                    ->helperText(translate('Enable if your business must apply timbre fiscal on invoices.')),
                TextInput::make('tunisian_stamp_duty_amount')
                    ->label(translate('Tunisian Stamp Duty Amount'))
                    ->numeric()
                    ->default(1000)
                    ->helperText(translate('Stored in minor currency units (default 1000).')),
            ])->columns();
    }
