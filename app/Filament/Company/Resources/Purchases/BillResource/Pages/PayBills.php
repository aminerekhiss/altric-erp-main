...
class PayBills extends ListRecords
{
        ...
        return [
            Actions\Action::make('processPayments')
                ...
                ->modalDescription(function () {
                    ...
                    return translate('You are about to pay') . " {$billCount} " . Str::plural(translate('bill'), $billCount) . ' ' . translate('for a total of') . " {$totalFormatted}. " . translate('This action cannot be undone.');
                })
                ->action(function () {
                    $data = $this->data;
                    $bankAccountId = $this->resolveBankAccountIdOrFail($data['bank_account_id'] ?? null);
                    ...
                    Notification::make()
                        ->title(translate('Bills paid successfully'))
                        ->body(translate('Paid') . " {$paidCount} " . Str::plural(translate('bill'), $paidCount) . ' ' . translate('for a total of') . " {$totalFormatted}")
                        ->success()
                        ->send();
    ...
    public function getSelectedBankAccount(): BankAccount
    {
        ...
        $bankAccount = BankAccount::query()
            ->when($companyId, fn (EloquentBuilder $query): EloquentBuilder => $query->where('company_id', $companyId))
            ->find($bankAccountId);

        return $bankAccount ?: BankAccount::query()
            ->when($companyId, fn (EloquentBuilder $query): EloquentBuilder => $query->where('company_id', $companyId))
            ->where('enabled', true)
            ->firstOrFail();
    }

    protected function resolveBankAccountIdOrFail(mixed $bankAccountId): int
    {
        $companyId = auth()->user()?->current_company_id;

        $bankAccount = BankAccount::query()
            ->when($companyId, fn (EloquentBuilder $query): EloquentBuilder => $query->where('company_id', $companyId))
            ->find((int) $bankAccountId);
