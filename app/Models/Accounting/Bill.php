...
class Bill extends Document
{
    ...
    public function syncFinanceEchanceSortie(): void
    {
        FinanceEchance::query()->updateOrCreate(
            ...
            [
                'reference' => (string) ($this->bill_number ?: ('BILL-' . $this->id)),
                'entry_type' => FinanceEchance::ENTRY_SORTIE,
                'amount' => (float) ($this->total ?? 0),
                'echance_date' => optional($this->due_date ?? $this->date)->toDateString() ?? company_today()->toDateString(),
                'supplier' => $this->vendor?->name,
                'is_paid' => $this->status === BillStatus::Paid,
            ]
        );
    }
