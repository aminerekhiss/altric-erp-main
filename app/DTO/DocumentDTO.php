...
readonly class DocumentDTO
{
    ...
    public function __construct(
        ...
        public string $total,
        public ?string $totalIncludingStamp,
    ...
    public static function fromModel(Document $document): self
    {
        ...
        $stampDuty = null;
        $totalIncludingStamp = null;

        $isTunisiaStampDutyEnabled = (bool) ($document->company->profile?->tunisian_stamp_duty_enabled ?? false);
        $supportsStampDuty = in_array($document::documentType(), [DocumentType::Invoice, DocumentType::RecurringInvoice], true);

        if ($isTunisiaStampDutyEnabled && $supportsStampDuty) {
            $stampDutyAmount = max((int) ($document->company->profile?->tunisian_stamp_duty_amount ?? 0), 0);
            $stampDuty = self::formatToMoney($stampDutyAmount, $currencyCode);
            $totalIncludingStamp = self::formatToMoney($document->total + $stampDutyAmount, $currencyCode);
        }
        ...
        return new self(
            ...
            totalIncludingStamp: $totalIncludingStamp,
            amountDue: $amountDue,
