...
readonly class DocumentPreviewDTO extends DocumentDTO
{
    public static function fromSettings(DocumentDefault $settings, ?array $data = null): self
    {
        ...
        $isTunisiaStampDutyEnabled = (bool) ($company->profile?->tunisian_stamp_duty_enabled ?? false);
        $supportsStampDuty = in_array($settings->type, [DocumentType::Invoice, DocumentType::RecurringInvoice], true);
        $stampDutyAmount = max((int) ($company->profile?->tunisian_stamp_duty_amount ?? 0), 0);

        $stampDuty = ($isTunisiaStampDutyEnabled && $supportsStampDuty)
            ? self::formatToMoney($stampDutyAmount, null)
            : null;

        $totalIncludingStamp = ($isTunisiaStampDutyEnabled && $supportsStampDuty)
            ? self::formatToMoney(95000 + $stampDutyAmount, null)
            : null;
